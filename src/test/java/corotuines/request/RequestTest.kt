...
class RequestTest {
    ...
    fun `Function does return the best student in the semester`() = runBlocking {
        ...
        kotlin.test.assertEquals(best, chosen)
    }
    ...
    fun `Cancellation works fine`() = runBlocking {
        ...
        job.cancel()
        delay(1000)
        kotlin.test.assertEquals(0, repo.returnedStudents)
    }
    ...
    fun `When one request has error, all are stopped and error is thrown`() = runBlocking {
        ...
        delay(1000)
        kotlin.test.assertEquals(0, repo.studentsReturned, "Looks like some requests were still running after the first one had an error")
    }
}
